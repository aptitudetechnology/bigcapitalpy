...
# Import get_date_range from the shared utils location
...
# New route for Profit & Loss Report
...
def profit_loss():
    ...
    # Fetch income accounts and calculate their balances
    ...
    for account in income_accounts:
        ...
        balance = db.session.query(func.sum(JournalLineItem.credit - JournalLineItem.debit)).join(
            JournalEntry
        ).filter(
        ...
        income_data.append({'name': account.name, 'balance': balance})
        total_income += balance

    # Fetch expense accounts and calculate their balances
    ...
    for account in expense_accounts:
        # Calculate balance for the period
        balance = db.session.query(func.sum(JournalLineItem.debit - JournalLineItem.credit)).join(
            JournalEntry
        ).filter(
            JournalLineItem.account_id == account.id,
            JournalEntry.organization_id == current_user.organization_id,
        ...
        expense_data.append({'name': account.name, 'balance': balance})
    ...
    report_period = {
        ...
        'end_date': end_date,
        'period_name': period.replace('_', ' ').title() # For display
    }
...
def discard_reconciliation(reconciliation_id):
    ...
    try:
        # Unmatch any matched transactions associated with this reconciliation
        matches = ReconciliationMatch.query.filter_by(reconciliation_id=reconciliation_id).all()
        for match in matches:
            if match.bank_transaction:
                match.bank_transaction.status = 'unmatched'
        ...
        flash('Reconciliation discarded successfully!', 'success')
        return redirect(url_for('financial.reconciliation'))
